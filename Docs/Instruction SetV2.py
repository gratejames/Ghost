...
def checkCommands():
	...
	if mne in unique_mnemonics:
		...
		return
	if not mne[-1].isalpha():
		print(f"Mnemonic cannot end in digit: '{mne}' at Index {hex(Index)}")
		return
...
with open('/home/flicker/.config/sublime-text/Packages/User/GHASM.sublime-syntax', 'w+') as f:
...
with open("cppDefs", "w+") as f:
	...
	for i in range(0, 0x100):
		...
		elif mne.startswith("STR0R"):
			content = setAtAddress(RR(), R0)
		elif mne.startswith("STR1R"):
			content = setAtAddress(RR(), R1)
		elif mne.startswith("STR2R"):
			content = setAtAddress(RR(), R2)
		elif mne.startswith("STR3R"):
			content = setAtAddress(RR(), R3)
