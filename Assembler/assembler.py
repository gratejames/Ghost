...
def intToHexString(num):
    if num > 0xFFFF:
        print(
            f"! Found the value {str(hex(num))}, which is greater than 0xffff. Rolling to {'0x' + ('0000' + str(hex(num))[2:])[-4:]}"
        )
...
class ramfs:
    ...
    def addFile(self, source, dest, flags):
        if source.endswith(".hex"):
            ...
            e = os.system(f"~/Desktop/Ghost/Lang/main.py '{source}' -fs")
            if e != 0:
                print("Compilation failed, unable to pack ramfs")
                exit(e)

            assembled = source.rsplit(".", 1)[0] + ".ghasm"

            subAssembler = assembler()
            subAssembler.loadFile(assembled)
            fileContents = " ".join(subAssembler.assemble(org=0, warnHLT=-1).split())
        else:
        ...
        folder["contents"].append(
            {
                ...
                "name": name,
                "flags": flagByte,
                "contents": [eval(x) for x in fileContents.strip().split(" ")],
            }
        )
    ...
    def saveFile(self, item):
        ...
        while cursor != len(fileContents):
            ...
            if (
                len(fileContents) - cursor > 127
            ):  # If we have more than one page, after this one
            ...
            for byte in range(cursor, min(cursor + 127, len(fileContents))):
                self.MEMORY[curPageAddr + byte % 127 + 1] = fileContents[
                    cursor + byte % 127
                ]
    ...
    def export(self):
        ...
        for i in range(4, min(36, 5 + self.freePages // 16)):
            for b in range(16):
                if i - 4 < self.freePages // 16 or (
                    i - 4 == self.freePages // 16 and self.freePages % 16 > b
                ):
                    self.MEMORY[i] = (self.MEMORY[i] << 1) + int(
                        self.bitmap[(i - 4) * 16 + b]
                    )
                else:
...
class assembler:
    TypeLengths = {
        ...
        "Instruction": 1,
        "Register": 0,
    }  # Length in bytes of each type used below
    ...
    def resolveData(self, line):
        ...
        if line.strip()[0] == ".":
            ...
            dataType = {".db": "Byte", ".ds": "String", ".dz": "Zeroes"}.get(
                dataTypeWord, None
            )
            if dataType is None:
                self.die(
    ...
    def resolveShorthand(self, line):
        ...
        if words[0] in AssemblerDefs.Shorthand.keys():
            ...
            ArgumentTypesString = (
                "None" if ArgumentTypes == [] else " ".join(ArgumentTypes)
            )
            if ArgumentTypesString in AssemblerDefs.Shorthand[words[0]].keys():
                ...
                self.die(
                    f"Expected types are: {self.prettyOrList(['[' + ', '.join(typeList.split(' ')) + ']' for typeList in listOfShorthandArguments])}"
                )
        return line
    ...
    def validateArguments(self, line):
        ...
        if words[0] in AssemblerDefs.Instructions.keys():
            ...
            if (
                not ArgumentTypesString
                == AssemblerDefs.Instructions[words[0]]["Arguments"]
            ):
                print(
                ...
                self.die(
                    f"Expected types are: [{', '.join(expectedArguments.split(' '))}]"
                )
        else:
    ...
    def getShared(self):
        ...
        for share in self.shares:
            ...
            try:
                ...
                self.die(
                    f"X Error sharing definition '{share}', could not be found in definitions"
                )
    ...
    def assemble(self, org=0, warnHLT=1, nested=0, definitions={}):
        ...
        if "HLT" not in self.fileContents and warnHLT == 1:
            ...
            print(
                "! A 'HLT' instruction found and one is not recommended: may result in unpredictable behavior"
            )
        ...
        for lineN, line in enumerate(linesList):
            ...
            self.position += sum(
                [
                    self.TypeLengths[typeOfItem]
                    for typeOfItem in self.argumentTypes(line)
                ]
            )
        ...
        for lineN, line in enumerate(linesList):
            ...
            for definition in self.definitions.keys():
                if definition in line:
                    ...
                    line = re.sub(
                        pat,
                        r"\g<1>" + str(intToHexString(self.definitions[definition])),
                        line,
                    )
