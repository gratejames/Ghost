...
class Function:
    ...
    def __repr__(self):
        return f"<{self.ret_type} {self.name}>"
    ...
    match state:
        ...
        case S.FUNC:
            if line.strip() == "}":
                ...
                state = S.BODY
                continue
            cur_func.lines.append(line.strip())
        case _:
            exit(1)
...
class parser:
    ...
    def memoryeffects(self, neweffects=None):
        if neweffects is not None:
            # self._memoryeffects = neweffects
            self.blocks[-1].memoryeffects = neweffects
        # return self._memoryeffects
    ...
    def parseFunction(self, f):
        ...
        # self.blocks = []
        # self.blocks = []
        self.blocks.append(nodes.Block(nodes.Label(f"{f.name}_%entry")))
        self.cur_func = f
        ...
        for line in f.lines:
            ...
            if line[0] == "%" and words[1] == "=":
                ...
                if op == "alloca":
                    ...
                    newnode = nodes.Load()
                    newnode.ty = words[1]
                    pt = nodes.Pointer(ty=words[4], address=words[5][:-1])
                    newnode.pointer = pt
                ...
                newnode.ty = words[3]
                newnode.pointer = nodes.Pointer(ty=words[3], address=words[4][:-1])
                self.addMemEffect(newnode.pointer.address, newnode)
                ...
                newnode = nodes.Ret(
                    ty=words[1], pre_node=self.resolvestring(words[1], words[2])
                )
                # f.ret = newnode
                f.ret = self.blocks[-1]
                f.ret.type = "return"
            ...
            elif (label := stripComments(line)).endswith(":"):
                newname = label[:-1]
                # print(
                ...
                if "; preds = " in line:
                    ...
                    preds = [f"{self.cur_func.name}_{name}" for name in preds]
                # print("New label", newname, len(self.blocks))
                # self.remove_unused_chains()
                # self.optimize_constant_chains()
                newlabel = nodes.Label(name=f"{self.cur_func.name}_%{newname}")
